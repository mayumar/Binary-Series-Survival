...
from models.training.data import rebalance_censoring, infer_training_context
from models.training.factories import build_lstm_model, build_loss, build_optimizer
...
def train_lstm(
    ...
    train_cfg = parse_training_config(raw_cfg, device_override=device_override)
    eval_cfg = parse_eval_config(raw_cfg)
    rebalance_cfg = parse_rebalance_config(raw_cfg)
    model_cfg = parse_lstm_model_config(raw_cfg)

    train_loader = rebalance_censoring(train_loader, rebalance_cfg)
    ctx = infer_training_context(train_loader, train_cfg.device)

    model = build_lstm_model(model_cfg, ctx)
    loss_fn = build_loss()
    optimizer = build_optimizer(model, train_cfg)

    evaluator = SurvivalEvaluator(loss_fn, eval_cfg)
    trainer = Trainer(train_cfg, evaluator)
